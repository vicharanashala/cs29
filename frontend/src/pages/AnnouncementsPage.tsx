  ...
  return (
    <div className="portal-page">
      <div className="portal-container" style={{ maxWidth: '860px' }}>
        ...
        {/* Logged Out Check */}
        {!isAuthenticated ? (
          <div className="portal-empty-state" style={{ padding: '40px 24px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)' }}>
            <div className="portal-empty-icon" style={{ background: 'var(--accent-glow)', color: 'var(--accent)' }}>
              <Megaphone size={32} />
            </div>
            <h2 className="portal-empty-title">Access Restricted</h2>
            <p className="portal-empty-desc">
              Please login first to view announcements, updates, and the SP Leaderboard.
            </p>
            <Link to="/login" className="btn-primary" style={{ textDecoration: 'none', display: 'inline-block', marginTop: '12px' }}>Login Now</Link>
          </div>
        ) : (
          <>
            {/* Announcements Feed */}
